  ...
  watchFiles() {
    ...
    for(var x = 0; x < atom.project.rootDirectories.length; x++) {
      for(var y = 0; y < configPaths.length; y++) {
          paths.push(configPaths[y].trim().replace(/\[root\]/g, atom.project.rootDirectories[x].path));
      }
    }
    watch(paths, { recursive: false }, function(evt, name) {
      ...
      if(evt === 'update' && (name.endsWith('war') || name.endsWith('jar') || name.endsWith('ear') || name.endsWith('sar'))) {
        let options = {};
        console.debug(name);
        atom.notifications.addInfo(name + ' is being deployed to Wildfly.', options);
        let destination = atom.config.get('atom-wildfly.wildflyHome') + '/standalone/deployments/'
        console.debug(name);
        console.debug(destination);
        copy(name, destination, (err, files) => {
          if(err) {
            atom.notifications.addError(err);
          }
          else {
            console.debug(files);
            atom.notifications.addSuccess('Successfully deployed archive: ' + files[0].dest);
          }
        })
      }
    });
