...
export default {
  ...
  config: {
    ...
    "configurationFile": {
      "description": "Specify your Wildfly configuration file if it isn't the default standalone.xml.",
      "type": "string",
      "default": process.env.JBOSS_HOME + '/standalone/configuration/standalone.xml'
    },
    "startCommand": {
      "description": "This is the path and command that is issued to start the server. The initial directory is your Wildfly Home.",
      "type": "string",
      "default": '/bin/standalone'
    },
    "managementPort": {
  ...
  startServer() {
    this.runCommand(this.getServerCommand(), [], (code) => {console.log(code);}, (data) => this.handleOutput(data), (data) => {console.error(data);});
  },
  ...
  runCommand (command, args, exit, stdout = {}, stderr = {}, options = {}) {
    if(this.serverProcess === null) {
      ...
      this.handleOutput('Server has already been started.');
  ...
  killServer() {
    ...
    if(this.serverProcess != null && this.serverProcess.started) {
      ...
      this.handleOutput('Server is not started.');
      console.debug(this.serverProcess);
    }
  },
  ...
  reStartServer() {
    if(this.serverProcess === null) {
      ...
      this.handleOutput('Server is already running.');
      console.debug(this.serverProcess);
    }
  },
